...
(() => {
    ...
    /**
     * Membuat sekawanan burung.
     * @param {object} opt
     * @param {HTMLElement} opt.parent  tempat lapisan burung ditambahkan
     * @param {number} opt.max          jumlah burung maksimum di layar
     * @param {string} [opt.extraClass] kelas tambahan (mis. warna hitam)
     * @param {number} [opt.every]      jeda munculnya burung baru (ms)
     * @param {boolean} [opt.keepFull]  selalu ganti burung yang keluar layar
     */
    const makeBirds = ({ parent, max, extraClass = '', every = 4500, keepFull = false }) => {
        let layer = null;
        let timer = null;

        const spawn = () => {
            if (!layer || document.hidden || layer.childElementCount >= max) {
                return;
            }
            ...
            anim.onfinish = () => {
                bird.remove();
                // jaga jumlah burung tetap penuh: ganti dengan burung baru
                if (keepFull && layer) {
                    setTimeout(spawn, rand(300, 1500));
                }
            };
        };

        const start = () => {
            if (reduceMotion || timer || !parent || !Element.prototype.animate) {
                return;
            }

            layer = document.createElement('div');
            layer.className = `bird-layer ${extraClass}`.trim();
            layer.setAttribute('aria-hidden', 'true');
            parent.appendChild(layer);

            // beberapa burung langsung muncul bergantian
            for (let i = 0; i < max; i++) {
    ...
    // Burung putih di seluruh undangan (maks. 3)
    const pageBirds = makeBirds({ parent: document.body, max: 3 });
    const startBirds = () => pageBirds.start();

    // Burung hitam di halaman pembukaan (maks. 6)
    const coverBirds = makeBirds({ parent: welcome, max: 6, extraClass: 'bird-layer-cover', every: 1800, keepFull: true });
    if (welcome) {
        const startCover = () => coverBirds.start();
        if (welcome.classList.contains('orn-play')) {
            startCover();
        } else {
    ...
    document.addEventListener('undangan.open', () => {
        ...
        setTimeout(() => coverBirds.stop(), 1200);
